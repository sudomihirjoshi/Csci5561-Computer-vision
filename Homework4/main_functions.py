...
def main_slp():
    ...
    accuracy_matrix = np.zeros((9,9))
    acc_i = 0
    acc_j = 0
    max_accuracy = 0
    lr_max = 0
    dr_max = 0
    # for lr in np.arange(0.008,0.016,0.0002):
    #     for dr in np.arange(0.92,1,0.01):
    #         w, b = train_slp(mini_batch_x, mini_batch_y,lr,dr)
    ...
    #                 acc = acc + 1
    #         accuracy = acc / num_test
    #         # accuracy_matrix[acc_i][acc_j] = accuracy
    #         acc_j += 1
    #         # print("#########  one pair of lr and dr tested  ###########")
    #         print("Lr,dr,accuracy ", lr,dr,accuracy*100)
    ...
    # print(lr_max,dr_max,max_accuracy)
    # w, b = train_slp(mini_batch_x, mini_batch_y,lr,dr)
...
def main_mlp():
    ...
    # for lr in np.arange(0.006,0.015,0.001):
    #     for num_iterations in np.arange(2000,15000,500):

    #         w1, b1, w2, b2 = train_mlp(mini_batch_x, mini_batch_y,lr,num_iterations)
    ...
    #         print("lr,iterations, accuracy = ", lr, num_iterations, accuracy*100)
    # print("best lr,iterations, accuracy = ", best_lr, best_iter, best_accuracy*100)
...
def main_cnn():
    ...
    #     for dr in np.arange(0.85,1.1,0.05):
    #         for iters in range(8000,11000,2000):
    ...
    for i in range(num_test):
        ...
        # print("shape of pred1 = ", np.shape(pred1))
        pred2 = relu(pred1)  # (14, 14, 3)
        # print("shape of pred2 = ", np.shape(pred2))
        pred3 = pool2x2(pred2)  # (7, 7, 3)
        # print("shape of pred3 = ", np.shape(pred3))
        pred4 = flattening(pred3)  # (147, 1)
        # print("shape of pred4 = ", np.shape(pred4))
        # print("shape of w = ", np.shape(w_fc))
        # print("shape of x = ", np.shape(pred4))
        ...
        if l_pred == label_test[0, i]:
            acc = acc + 1
    accuracy = acc / num_test
    # print("lr,dr,accuracy,iters = ", lr,dr,accuracy,iters)
    for i in range(10):
        confusion[:, i] = confusion[:, i] / np.sum(confusion[:, i])
